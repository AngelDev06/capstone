...
path_portion="llvm-capstone/llvm"

if ! echo "$2" | grep -q -w "$path_portion" ; then
  echo "[x] $2 is an invalid path (it should have at least somewhere the \"llvm-capstone/llvm\" substr for it to work)"
  pause
fi
